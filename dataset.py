...
class Dataset(object):
    ...
    def __init__(self, questions, answers, input_vocabulary, output_vocabulary):
        ...
        if len(questions) != len(answers):
            raise RuntimeError(
                "questions and answers lists must be the same length, as they are lists of input-output pairs.")
    ...
    def train_val_split(self, val_percent=20, random_split=True, move_samples=True):
        ...
        validation_dataset = Dataset(validation_questions, validation_answers, self.input_vocabulary,
    ...
    def sort(self):
        ...
        """
        if self.size() > 0:
            self.questions_into_int, self.answers_into_int = zip(
                *sorted(zip(self.questions_into_int, self.answers_into_int),
    ...
    def save(self, filepath):
        ...
        with open(questions_filepath, mode="w", encoding="utf-8") as file:
            for question_into_int in self.questions_into_int:
                question = self.input_vocabulary.ints2words(question_into_int, is_punct_discrete_word=True,
                                                            capitalize_i=False)
                file.write(question)
                file.write('\n')
        ...
        with open(answers_filepath, mode="w", encoding="utf-8") as file:
            for answer_into_int in self.answers_into_int:
                answer = self.output_vocabulary.ints2words(answer_into_int, is_punct_discrete_word=True,
                                                           capitalize_i=False)
                file.write(answer)
                file.write('\n')
